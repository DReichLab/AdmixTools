...
main (int argc, char **argv)
{
  ...
  printf("numeg: %d\n", numeg) ; 
  npops  = numlines(poplistname) ; 
  ZALLOC(pops, npops, char *) ; 
  npops = loadlist(pops, poplistname) ;
  if (nqlist==0) { 
  ...
  for (a=1 ; a<npops ; ++a) { 
   for (b=q ; b<npops ; ++b) { 
    qlist[0][numfs] = qlist[2][numfs] = pops[0] ; 
    qlist[1][numfs] =  pops[a] ; 
    qlist[3][numfs] =  pops[ab ; 
    ++numfs ; 
   }
  }
