...
void eigxsym(double *evec, double *eval, 
...
#define TDV NMAX
 ...
 if (n>NMAX) fatalx("(eigxsym) n too large\n") ;
...
#define IMAG(A)  A.im 
 ...
 if (n>NMAX) fatalx("(eigx) n too large\n") ;
...
void svdx(double *X, int m, int n, double *U, double *S, double *VT, int *nval)
...
{  
  ...
  if (n>NMAX) fatalx("(svdx) n too large\n") ;
  if (m>MMAX) fatalx("(svdx) m too large\n") ;

  for(k=0; k<m; k++) copyarr(X+k*n, aX[k], n);

  f02wec(m, n, (double *) aX, (Integer) TDA, 0,
    (double *) dummyaB, (Integer) TDB, (Boolean) TRUE, (double *) dummyaU, (Integer) TDQ,
    (double *) S, (Boolean) TRUE, (double *) aVT, (Integer) TDPT, iter,
	 (double *) dummyE, &failinfo, &fail);

  printf("f02wec ran and produced fail parameter %d\n",fail);

  for(a=0; a<m; a++)
  {
    for(b=0; b<n; b++) U[a*n+b] = aX[a][b];
  }

  for(a=0; a<n; a++)
  {
    for(b=0; b<n; b++) VT[a*n+b] = aVT[a][b];
  }

  nvalval = 0;
  for(k=0; (k<m) && (k<n); k++)
  {
    if(S[k] > 1.0e-8) nvalval++;
  }
  *nval = nvalval;
...
#define IMAG(A)  A.im 
 ...
 if (n>NMAX) fatalx("(linsolv) n too large\n") ;
 ...
 f04arc(n, (double *) amat, (Integer) TDA, trhs, 
  ans, &fail) ;
