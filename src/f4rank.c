...
int dofrankfix(int m, int n, int rank, int nfix) 
{
 int x, t, i ;  


 if (nfix > rank) return 0 ; 
 if (rank >= m) return 0 ; 
 if (rank >= n) return 0 ; 
 if (rank == 0) return m*n ; 
 t = 0 ;
...
void doranktestfix(double *mean, double *var, int m, int n, int xrank, F4INFO *f4pt, int *vfix) 
{
...
void printstrmat(char **ss, double *mat, int m, int n) 
{
 int a ;
 double *x, *scale, y ;  
...
int solvitforcez(double *coeffs, double *rhs, int dim, double *ans, int *vl, int nf)  
{
  int *vmap, *vmiss ; 
...
*/
  ...
  if (verbose) printf("init scores:  %9.3f %9.3f\n", y0, y1) ; 
...
// ret = solvitfix(coeffs, rhs, adim, ans, vl, vfl, nf) ; 
   ...
   mulmat(xmean, A, B, m, rank, n) ;
   y1 = scx(varinv, mean, xmean, d) ;
   if (verbose) printf("iter A: %d %9.3f\n", iter, y1) ; 
  }

  if (pA != NULL) copyarr(A, pA, adim) ;
  if (pB != NULL) copyarr(B, pB, bdim) ;
...
double ranktest(double *mean, double *var, int m, int n, int rank, double *pA, double *pB) 
{
  ...
  if (rank == 0) {
   ...
   tail = rtlchsq(d, T2) ;
   if (verbose) printf("T2: %9.3f  tail: %15.9g\n", T2, tail) ;
   free(ww) ;
   free(varinv) ;
   return T2 ;
  }
...
void 
 f4info_init(F4INFO *f4pt, int nl, int nr, char **popllist, char **poprlist, int rank) 
{
