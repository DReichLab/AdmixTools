...
void getextlabel(NODE *node, char *sss) ;  
...
void findvname(char *ss, int n) 
{
  ...
  for (;;) { 
   ...
   if (indxstring(vnames, numvertex, sss) < 0) break ;
   ++x ;
  }
...
void findename(char *ss, int n) 
{
  ...
  for (;;) { 
   ...
   if (indxstring(enames, numedge, sss) < 0) break ;
   ++x ;
  }
...
void getvmind(int ind, int *adind, int *wind) 
{
 int t, k, j, l, z, lo, hi ; 
 NODE *node ;
...
int getnumadmix() 
{
 if (numadmix < 0) fatalx("graph not initialized\n") ;
 return numadmix ;
}
int getnumvertex() 
{
 if (numvertex < 0) fatalx("graph not initialized\n") ;
 return numvertex ;
}
int getnumedge() 
{
 if (numedge < 0) fatalx("graph not initialized\n") ;
 return numedge ;
}
...
void getvwts(double *pwts, int *nrows, int *nedge) 
...
int getpopinfo(char *name, int *popsize, int num)
{
   NODE *node ; 
   int k ;
...
void getpnwts(double *pwts, int *nrows, int *nedge) 
...
void destroyg()
{
 ...
 if (adlist != NULL) free(adlist) ;
 if (xadlist != NULL) free(xadlist) ;
...
void setispath() 
{
 ...
 for (;;) { 
  ...
  b = intsum(w1, nn) ;  
  if (a==b) break ;
  if (a<b) fatalx("badbug A\n") ;
  if (b>nn) fatalx("badbug B\n") ;
  a = b ;
 }
 if (ispath != NULL) free(ispath) ; 
...
// not clipped
{
 ...
 for (a=0; a<numedge; a++) {
  edge = &elist[a] ;
  node = (NODE *) edge -> up ; u = node -> gnode ;
  node = (NODE *) edge -> down ; v = node -> gnode ;
  x[u*n+v] += 1 ;  
 }
...
{ 
  ...
  int t, k, tbase ;
    ...
    for (k=0; k<nt; ++k) {
     ...
     if (t == tbase) fatalx("bad admix (recursion) %s\n", vertname) ;
     node -> windex[k] = t ;
     ++(node -> numwind) ;
    }
...
// printf("##deleting %s\n", node -> name) ;
   ...
   if ((NODE *) nparent -> left == node) { 
    nparent -> left = NULL ;
    nparent -> eleft = NULL ;
    if (nparent -> right == NULL) nparent -> isleaf = YES ;
   }

   if ((NODE *) nparent -> right == node) { 
    nparent -> right = NULL ;
    nparent -> eright = NULL ;
    if (nparent -> left == NULL) nparent -> isleaf = YES ;
   }
   node -> left = node -> right = NULL ;
   node -> eleft = node -> eright = NULL ;
  }
  if (nkill > 0) return gsimplify(n+nkill) ;
  ...
  for (i=0; i<numvertex; ++i) {  
    ...
    for (k=0; k<l; ++k) {
     ...
     if (node -> right != NULL) ++ecount[i] ;
     if (node -> left != NULL) ++ecount[i] ;
  }
// kill edges with one outedge
  for (i=0; i<numvertex; ++i) {  
    ...
    if (node -> isdead) continue ;  
    if (node -> numwind>0) continue ;  
    if (ecount[i] != 1) continue ; 
    ++nkill ;
//  printf("##deleting %s\n", node -> name) ;
    node -> isdead = YES ;
...
void addlabel(char *label, char *vertname) 
{
  NODE *node ;
  int t, i ;
...
void addvertex(char *vertname)  
{
  ...
  if (vertname==NULL) fatalx("bad addvertex call\n") ;
  t = vertexnum(vertname) ;
  if (t>=0) return ;
...
void addedge(char *ename, char *x1, char *x2)
{
  EDGE *edge ;
  ...
  if ((node -> eright) != NULL) fatalx("> 2 edges at %s\n", node -> name) ;
...
int loadgraph(char *rname, char ***peglist)          
{
  int maxnodes = MAXG ;
  int numeg, k ;
  NODE *node ;
 ...
 while (fgets(line, MAXSTR, fff) != NULL)  {
   ...
   if (kret == 0) {
    ...
    if (nsplit>=5) {    
     sx = spt[4] ;
     t = strcmp(sx, "zero") ;
     if (t==0) edge -> iszero = YES ;
     if (!isalpha(sx[0])) edge -> val = atof(sx) ;
    }
    v1 = vindex(s1, vlist, n) ;
    v2 = vindex(s2, vlist, n) ;
    if (v1<0) fatalx("bad edge: %s\n", line)  ;
    if (v2<0) fatalx("bad edge: %s\n", line)  ;
    node1 = &vlist[v1] ;
    if (node1 -> left != NULL) fatalx("two left edges! %s\n", node1 -> name) ;
    node2 = &vlist[v2] ;
    node = &vlist[v1] ; 
    node -> eleft = (struct EDGE *) edge ;
    edge -> isleft = YES ;
   ...
   if (kret == 0) {
    ...
    if (v2<0) fatalx("bad edge: %s\n", line)  ;
    node1 = &vlist[v1] ;
    if (node1 -> right != NULL) fatalx("two right edges! %s\n", node1 -> name) ;
    node2 = &vlist[v2] ;
    node = &vlist[v1] ; 
    node -> eright = (struct EDGE *) edge ;
    node = &vlist[v2] ; 
   ...
   if (kret==0) {
    ...
    s1 = spt[1] ;
    v1 = vindex(s1, vlist, n) ;
    if (v1<0) fatalx("bad admix: %s\n", line)  ;
    w = 0 ;
    node = &vlist[v1] ; 
    for (k=0; k<nt; ++k) {
     s2 = ssx[k] ;
     v2 = vindex(s2, vlist, n) ;
     if (v2<0) fatalx("bad admix: %s\n", line)  ;
     if (v1 == v2) fatalx("bad admix (recursion) %s\n", line) ;
     node -> windex[k] = v2 ;
     ++(node -> numwind) ;
    }
    copyarr(ww, node -> wmix, nt) ;
...
void dumpdotgraph(char *graphdotname) 
// in dot format 
{
...
/**
   for (k=0; k<numvertex; ++k) {  
    kk = ind[k] ;
    node = &vlist[kk] ;
    freestring(&sss);  sss=strdup(node -> name) ;  substring(&sss, ":", "_") ;
...
void pedge(FILE *fff, NODE *anode, NODE *bnode, double val, int mode)  
{
 ...
 substring(&ss2, ":", "_") ;
 fprintf(fff, "%s -> ", ss2) ;             
 freestring(&ss3);  ss3=strdup(bnode -> name) ;  substring(&ss3, ":", "_") ;
 fprintf(fff, "%s ", ss3) ;           
...
void dumpgraph(char *graphname) 
{
   ...
   if (hires) strcpy(sform, " %10.4f") ;
   else  strcpy(sform, " %9.3f") ;
   if (graphname == NULL) fff = stdout ;
   else openit(graphname, &fff, "w") ;
...
*/
 ...
 //1 dump vertex
   for (k=0; k<numvertex; ++k) {  
    kk = ind[k] ;
    node = &vlist[kk] ;
    if (node -> distance == HUGEDIS) fprintf(fff, "##") ;
    fprintf(fff, "vertex %12s %9.0f\n", node -> name, node -> time) ;
   }
 //2 dump label
   for (k=0; k<numvertex; ++k) {  
...
// dump ledge, redge 
   ...
   if  (graphname != NULL) fclose(fff) ;
   free (dd) ; 
   free (ind) ;
}
double exnames(char **pup, char **pdown, char **pename, int numedge) 
/*up down edgename for edgenum */
{
  ...
  *pename = strdup(edge -> name) ; 
  node = (NODE *) edge -> up; *pup = strdup(node -> name) ;
  node = (NODE *) edge -> down; *pdown = strdup(node -> name) ;
//printf("ename: %s  up:  %s\n", *pename, *pup) ;
  return (edge -> val) ;         
...
addnode(char *nodename, char *edgename, double breakval) 
{
   ...
   t = edgenum(edgename) ;
   if (t<0) fatalx("edge: %s not found\n", edgename) ;
   edge = &elist[t] ;
   node1 = (NODE *) edge -> up ;
...
void addmixedge2(char **nodes, char **edgenames, char *newedgename, 
 char *label, double *breakval, double eval, double *wmix, double *eeval, int nadmix) 
// label can be NULL
...
int setxx(NODE *node, NODE **xx) 
{
  ...
  nn = (NODE *) node -> parent ; if (nn!= NULL) { xx[t] = nn ; ++t ; } 
  nn = (NODE *) node -> left ; if (nn!= NULL) { xx[t] = nn ; ++t ; } 
  nn = (NODE *) node -> right ; if (nn!= NULL) { xx[t] = nn ; ++t ; } 
...
void reroot(char *nodename) 
{
   ...
   node  = vert(nodename) ;
   if (node == NULL) fatalx("node: %s not found\n", nodename) ;
   oldroot = root() ;
   printf ( "oldroot: %s\n", oldroot -> name) ;
   nee = setxx(node, xx) ;  // neighbours
   if (nee==3) fatalx("3 edges from new root: %s\n", nodename) ;
/* set distances */
   setdistances(node) ;
   oldroot -> isroot = NO ; 
   node -> isroot = NO ; 
...
{  
   ...
   oldroot = root() ;
   setdis(node, 0) ;
   if (oldroot -> distance > 1000) fatalx("%s must be in base tree\n", node -> name) ;
   numiter = 0 ;
   for (;;) {  
    nset = 0 ;
    ++numiter ;
    if (numiter>20) fatalx("looping...\n") ;
    unset =  NO ;  // vertex unset ??  
    for (k=0; k<numvertex; ++k) { 
      tnode = &vlist[k] ;
      if (tnode -> isdead) continue ;
      nw = tnode -> numwind ;
      if (nw == 0) continue ;
      if (tnode -> distance < HUGEDIS) continue ;
      for (j=0; j<nw; ++j) { 
   ...
   if (unset) fatalx("node not linked to root... %s\n", unode -> name) ;
}
...
void fixedge(EDGE *edge) 
{
  ...
  if (n1->distance < n2 -> distance) return ;
...
// fix up edges 
{
  ...
  for (j=0; j<nee; ++j) {  
   ...
   if (qleft) node -> right = (struct NODE *) tnode ;
   if (!qleft) node -> left = (struct NODE *) tnode ;
   qleft  = YES ;
  }
...
// find edge linking two vertices
{
  ...
  for (k=0; k<numedge; ++k) {  
   ...
   x1 = (NODE *) edge -> up ;
   x2 = (NODE *) edge -> down ;
   if ((n1 == x1) && (n2 == x2)) return edge ;
   if ((n1 == x2) && (n2 == x1)) return edge ;
  }
  return NULL ;
}
void setdis(NODE *node, int dis) 
...
void supersetup(int *xvlist, int *nxvlist, int **xelist, int *nxelist, int **admixv, int *admixedge, int *nxalist)  
{
  ...
  for (k=0; k< numadmix; ++k)  {  
   ...
   t = node -> numwind ;
   if (t <= 0) fatalx("bad logic\n") ;
   copyiarr(node -> windex, admixv[nxa], t) ;

   if (node -> left == NULL) fatalx("bad admix node %s\n", node -> name) ;
   if (node -> right != NULL) fatalx("bad admix node (redge) %s\n", node -> name) ;
   edge = (EDGE *)  node -> eleft ;
...
void superest(double *xmean, double *xvar, double *svar, double *yobs,  int nxvlist, int *eelist) 
{
   double tiny = 1.0e-12 ;
   int x, a, t ; 
...
int findlabel(char *label)                
{
 ...
 for (k=0; k<numvertex; ++k) { 
  ...
  lab = node -> label ;
  if (lab == NULL) continue ; 
  if (strcmp(lab, label) == 0) return k ;
 }
 return -1 ;
...
void copystringsd(char **eglist, char **neweglist, int numeg, int  xdel)
{
    ...
    if (xdel<0) fatalx("yukk\n") ;
    if (xdel>=numeg) fatalx("yukk\n") ;
    j = 0 ;
    for (i=0; i<numeg; ++i) {
     if (i==xdel) continue ;
