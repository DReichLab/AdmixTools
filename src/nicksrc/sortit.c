...
void lsortit(long *a, int *ind, int len) 
{
  int i,k  ;
  int *inda ;

  if (len==0) fatalx("(lsortit) len = 0\n") ;
  ...
  copylarr(a,lttt,len) ;
  qsort((int *) inda, len, sizeof(int), (int (*) (const void *, const void *)) lcompit);
