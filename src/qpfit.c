...
double T2score(double *coeff, double *ymean, double *yvar, int *ktable, int nl, int nr) ;  
...
{
    int a, b, dim, k, ret, dd ;  
    int a1, b1, a2, b2, k1, k2 ;
    double y, yy, y1, y2 ; 
    double *zmean, *zvar, *zinv ;


    dim = nl * nr;
    ZALLOC(zmean, nr, double) ; 
    ZALLOC(zvar, nr*nr, double) ; 
    ZALLOC(zinv, nr*nr, double) ; 
	...
	k2 = ktable[a2 * nr + b2];
        y2 = yvar[k1*dim + k2] ;
        zvar[b1*nr+b2] += coeff[a1]*coeff[a2]*y2 ;  
    }}}}
    pdinv(zinv, zvar, nr) ; 
    yy = scx(zmean, NULL, zinv, nr) ; 
    free(zmean) ; 
    free(zvar) ; 
    free(zinv) ;
