...
void f3scz(double *estn,  double *estd, SNP *cupt, Indiv **indm,  
  int *xindex, int *xtypes, int nrows, int type1, int type2, int type3) 
// processes X chromosome correctly
{
   int k, g, i, a, b  ; 
...
// processes X chromosome correctly
{
   ...
   f3scz(estn, estd, cupt, indm, xindex, xtypes, nrows, type1, type2, type3) ;
...
//     f4y(&ytop,  cupt, xindex, xtypes, nrows, a, b, c, d) ;
       if (aafreq[a] < -1.0) continue ;
       if (aafreq[b] < -1.0) continue ;
       if (aafreq[c] < -1.0) continue ;
       if (aafreq[d] < -1.0) continue ;
...
void oldwjackest(double *est, double *sig, double mean, double *jmean, double *jwt, int g)  
// test for jwt 0 funky stuff OOD
{
  ...
  m = 0 ; 
  mmean = mean ;
  if (jackweight == NO) vclear(jjwt, 1.0, n) ;
  if (quartile > 0.0) { 
   ZALLOC(ord, n, int) ;
   sortit(jjmean, ord, n) ; 
   dpermute(jjwt, ord, n) ; 
...

{
    ...
    if (bbot == NULL) vclear(wbot, 1.0, nblocks) ;
    else copyarr(bbot, wbot, nblocks) ;
