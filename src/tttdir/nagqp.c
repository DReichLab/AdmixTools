...
void
myfun (Integer n, double *xc, double *fc, Nag_Comm * comm)
{
  int nparams = n;
  double *params;
  double y, yval, yfix;
...
double
nagopt1 (double lo, double hi, double *plam, int wind)
{
  ...
  e04abc (nagxfun, e1, e2, &tlo, &thi, max_fun, &minp, &minval, &comm, &fail);
...
void
nagxfun (double xp, double *nagxval, Nag_Comm * comm)
{
  double y;
