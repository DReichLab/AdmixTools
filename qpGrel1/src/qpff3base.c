...
int doff3(double *ff3, double *ff3var, SNP **xsnplist, int *xindex, int *xtypes, 
...
void  dumpit(char *dumpname, double *ff3, double *ff3var, char **eglist, int numeg) ;
...
int main(int argc, char **argv)
{
 ...
 for (i=0; i<numeg; i++) {  
  ...
  if (outpop == NULL) outpop = strdup(eglist[0]) ;
...
// differences from basepoint 
  x = NO ;    ; 
  if (fstdmode) x = 2 ;
  scale = dofstnumx(fst, f2, f2sig, xsnplist, xindex, xtypes, nrows, ncols, numeg, nblocks, indivmarkers, x) ;     
  ...
  kk = doff3(ff3, ff3var, xsnplist, xindex, xtypes, nrows, ncols, numeg, nblocks, lambdascale) ;     
